...
# ---- case features ----
# 0=pad, 1=lowercase, 2=UPPERCASE, 3=Titlecase, 4=has_digit, 5=other
...
def run_epoch(model, loader, opt, crit, dev, clip=5.0):
    ...
    for wids, cids, tids, lens, _ in loader:
        ...
        opt.zero_grad()
        logits = model(wids, cids, lens.to(dev))
...
def main():
    pa = argparse.ArgumentParser(description='Task 2: BiLSTM + GloVe + case features')
    ...
    pa.add_argument('--model_file', default='blstm2.pt')
    ...
    if args.mode == 'train':
        ...
        # expand vocab with dev/test words that have GloVe vectors
        ...
        model = GloVeNERTagger(len(w2i), len(t2i), emb_mat).to(device)
        ...
        # create model with dummy embedding matrix, state_dict will overwrite
        dummy_mat = np.zeros((len(w2i), 100), dtype=np.float32)
        model = GloVeNERTagger(len(w2i), len(t2i), dummy_mat,
                               n_cases=ckpt.get('n_cases', N_CASES),
                               case_dim=ckpt.get('case_dim', CASE_DIM)).to(device)
        model.load_state_dict(ckpt['model'])
        ...
        preds = predict_all(model, test_dl, i2t, device)
        write_output(preds, 'test2.out')
        print('wrote dev2.out, test2.out')
